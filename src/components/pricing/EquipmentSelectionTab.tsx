...
export function EquipmentSelectionTab({
  ...
  const [selectedEquipment, setSelectedEquipment] = useState<EquipmentRate | null>(null);
  const [quantity, setQuantity] = useState(1);
  const [duration, setDuration] = useState(1);
  const [isLoading, setIsLoading] = useState(false);

  const equipmentDetails = details.filter((d) => d.pricing_type === "equipment");
  ...
  const loadEquipmentRates = async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase
        .from("equipment_rates")
        .select("id, name, name_ar, category, unit, rental_rate, hourly_rate, includes_operator, includes_fuel")
  ...
  const filteredEquipment = equipmentRates.filter(
    (e) =>
  ...
  const handleAdd = async () => {
    ...
    const success = await onAdd({
      pricing_type: "equipment",
      resource_id: selectedEquipment.id,
      resource_name: selectedEquipment.name_ar || selectedEquipment.name,
      unit: selectedEquipment.unit,
      unit_price: selectedEquipment.rental_rate,
      quantity: quantity,
      duration: duration,
    });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-medium text-muted-foreground">المعدات المختارة</h3>
      ...
      {equipmentDetails.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground border-2 border-dashed rounded-lg">
          <p>لا توجد معدات مضافة</p>
          <p className="text-xs mt-1">اضغط على "إضافة معدة" لاختيار من المكتبة</p>
        </div>
      ) : (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">معدل الإيجار</TableHead>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <ScrollArea className="h-[300px] border rounded-lg">
              {isLoading ? (
                ...
                <div className="flex items-center justify-center h-full">
                  <p className="text-muted-foreground">لا توجد معدات متاحة</p>
                </div>
              ) : (
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {equipment.category}
                            {equipment.includes_operator && " • شامل السائق"}
                            {equipment.includes_fuel && " • شامل الوقود"}
                          </p>
                        </div>
                        <div className="text-left">
                          <p className="font-medium text-primary">
                            {formatNumber(equipment.rental_rate)} {currency}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            / {equipment.unit}
                          </p>
            ...
            {selectedEquipment && (
              <div className="flex items-center gap-4 p-4 bg-muted rounded-lg">
                <div className="flex-1">
                  <p className="font-medium">
                    {selectedEquipment.name_ar || selectedEquipment.name}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {formatNumber(selectedEquipment.rental_rate)} {currency} / {selectedEquipment.unit}
                  </p>
                </div>
                ...
                <div className="text-left min-w-[120px]">
                  ...
                  <p className="font-bold text-primary">
                    {formatNumber(selectedEquipment.rental_rate * quantity * duration)} {currency}
                  </p>
                </div>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd} disabled={!selectedEquipment}>
              إضافة
            </Button>
          </DialogFooter>
        </DialogContent>
